...
--
-- Host: localhost
-- Generation Time: Jun 14, 2009 at 03:24 PM
...
DROP TABLE IF EXISTS `kac_tbkeywords`;
...
DROP TABLE IF EXISTS `kcb_tbcities`;
...
DROP TABLE IF EXISTS `kcb_tbcontinents`;
...
DROP TABLE IF EXISTS `kcb_tbcountries`;
...
DROP TABLE IF EXISTS `kcb_tbstates`;
...
CREATE VIEW `customer_product_dollarsales` AS select `customers`.`customerName` AS `customerName`,`products`.`productName` AS `productName`,`products`.`productLine` AS `productLine`,(`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`) AS `dollar_sales` from (((`customers` join `orders`) join `orderdetails`) join `products`) where ((`orders`.`customerNumber` = `customers`.`customerNumber`) and (`orders`.`orderNumber` = `orderdetails`.`orderNumber`) and (`products`.`productCode` = `orderdetails`.`productCode`));
